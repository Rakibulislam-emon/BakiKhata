    ...
    <div className="relative space-y-6 pb-32 pt-0">
      ...
      <div className="relative z-10 space-y-6">
        ...
        >
          ...
          <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/10 rounded-full blur-3xl -mr-20 -mt-20 group-hover:bg-indigo-500/20 transition-all duration-1000 pointer-events-none" />
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div className="flex-1">
              <div className="flex items-center gap-4 mb-3">
                <m.button
                  ...
                  whileTap={{ scale: 0.9 }}
                  onClick={() => router.back()}
                  className="w-10 h-10 rounded-xl bg-white/50 dark:bg-slate-800/50 flex items-center justify-center text-slate-500 dark:text-slate-400 border border-slate-200/50 dark:border-white/5 shadow-sm hover:text-indigo-500 transition-colors"
                  aria-label="পেছনে যান"
                >
                  <ChevronLeft className="w-6 h-6" />
                </m.button>
                <div className="w-12 h-12 rounded-2xl bg-indigo-500/10 flex items-center justify-center ring-1 ring-indigo-500/20 backdrop-blur-sm">
            ...
            <div className="w-full md:w-auto min-w-[320px]">
              <div className="relative group/search mr-10">
                    <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 text-slate-400 w-5 h-5 group-focus-within/search:text-indigo-500 transition-colors duration-300" />
                    <input
                      type="text"
                      placeholder="গ্রাহকের নাম খুঁজুন..."
                      value={searchTerm}
                      onChange={(e) => setSearchTerm(e.target.value)}
                      className="w-full pl-12 pr-12 py-3 rounded-2xl border border-slate-200/60 dark:border-white/5 bg-white/50 dark:bg-slate-800/50 backdrop-blur-xl shadow-sm focus:bg-white dark:focus:bg-slate-800 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all duration-300 placeholder:text-slate-400 font-bold text-slate-700 dark:text-slate-200"
                    />
                    <AnimatePresence>
                      {searchTerm && (
                      ...
                      <m.button
                        ...
                        className="min-w-[44px] min-h-[44px] flex items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-700 text-slate-400 hover:text-indigo-500 transition-colors shadow-sm"
